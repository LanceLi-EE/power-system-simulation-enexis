...
class GraphProcessor:
    ...
    def find_alternative_edges(self, disabled_edge_id: int) -> List[int]:
        ...
        """
        # Disabled edge is not valid edge id
        if disabled_edge_id not in self.edge_ids:
            raise IDNotFoundError
        
        # Disabled edge is already disabled
        if self.edge_enabled[disabled_edge_id - 1] == 0:
            raise EdgeAlreadyDisabledError

        # Disable the edge and check if graph is still connected
        if self.edge_enabled[disabled_edge_id - 1] == 1:
            G_1 = self.G
            G_1.remove_edge(self.edge_vertex_id_pairs[disabled_edge_id -1][0], self.edge_vertex_id_pairs[disabled_edge_id - 1][1])
            if nx.is_connected(G_1):
                return []
            else: # graph is not connected, find alternative edges
                alternative_edges = []
                for id, enabled, (u,v) in zip(self.edge_ids, self.edge_enabled, self.edge_vertex_id_pairs):
                    if not enabled:
                        G_2 = G_1
