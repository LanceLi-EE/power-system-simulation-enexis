...
class power_grid_calculation:
    ...
    #aggregate power flow results in 2 tables 
    '''
    def aggregate_power_flow_results(self):
        output_data = self.output_data
        column_names = ["Timestamp", "Maximum p.u voltage", "Node ID with Maximum p.u voltage", "Minimum p.u voltage", "Node ID with Minimum p.u voltage"]
        maximum_pu_voltage = []
        nodeIDwithMaximum_pu_voltage = []
        minimum_pu_voltage = []
        nodeIDwithMinimum_pu_voltage = []
        for index in output_data.index:
            maximum_pu_voltage.append(np.max(output_data.loc[output_data.index == index, "Voltage Magnitude (p.u)"]))
            nodeIDwithMaximum_pu_voltage.append(output_data.loc[output_data.index == np.max(output_data.index), "Node ID"])
            minimum_pu_voltage.append(np.min(output_data.loc[output_data.index == index, "Voltage Magnitude (p.u)"]))
            nodeIDwithMinimum_pu_voltage.append(output_data.loc[output_data.index == np.min(output_data.index), "Node ID"])
        
        table = DataFrame(list(zip(column_names, maximum_pu_voltage, nodeIDwithMaximum_pu_voltage, minimum_pu_voltage, nodeIDwithMinimum_pu_voltage)))
        return table
    '''
